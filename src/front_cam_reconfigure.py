...
# example
outside_area_polygon = [(0.0, 0.0), (0.0, 3.0), (3.0, 3.0), (3.0, 0.0)]
# ramp_area_polygon = [(-8.5, 10.7), (-4.1, 10.7), (-4.1, 1.4), (-8.5, 1.4)]

class FrontCamReconfigureNode:
    def __init__(self):
        rospy.init_node('front_cam_reconfigure_node')
        ...
        rospy.loginfo("front cam reconfigure node started")
        ...
        self.global_rgbd_obstacle_layer_front_cam_reconfigure_client = dynamic_reconfigure.client.Client('move_base/global_costmap/rgbd_obstacle_layer_front_cam', timeout=30)
        self.local_rgbd_obstacle_layer_front_cam_reconfigure_client = dynamic_reconfigure.client.Client('move_base/local_costmap/rgbd_obstacle_layer_front_cam', timeout=30)
    ...
    def reconfigure_obstacle_layer(self, enable):
        ...
        self.global_rgbd_obstacle_layer_front_cam_reconfigure_client.update_configuration(params)
        self.local_rgbd_obstacle_layer_front_cam_reconfigure_client.update_configuration(params)

    def check_is_in_outside_area(self, position):
        # Check if the given position is inside the ramp area polygon
        x, y = position.x, position.y
        inside = self.point_inside_polygon(x, y, outside_area_polygon)
        return inside
